...
parser.add_argument("--cover_image_path", type=str, default="images/wsz.jpg")
...
pipe_attacker, forward_scheduler, inverse_scheduler = imprint_utils.load_attacker_pipe(modelid=args.modelid_attacker,
...
for step in tqdm.tqdm(range(args.steps)):
    ...
    if step % args.validation_steps == 0:
        ...
        with torch.no_grad():
            ...
            image_from_z0.save(os.path.join(out_dir, f"attack_instance_step={step}.png"))
            
            psnr = psnr_PIL(image_from_z0, cover)
            accuracy_results = wm_provider.get_accuracies(inverted_latent.detach())
            bit_accuracy = accuracy_results["bit_accuracies"][0] if "bit_accuracies" in accuracy_results else 0.0
