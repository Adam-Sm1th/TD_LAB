...
with torch.no_grad():
    ...
    message_bits_str_initial = wm_initial_results["message_bits_str_list"][
        0] if "message_bits_str_list" in wm_initial_results else None
    ...
    detection_successful = check_if_detection_successful(wm_type=args.wm_type,
                                                         ...
                                                         value=results["bit_accuracy"] if args.wm_type == "GS" else
                                                         results["p_value"])
    ...
    # log
    print(
        f"(Benign image) detection_success: {detection_successful}, bit accuracy: {results['bit_accuracy']:.5f}, p_value: {results['p_value']}")

    # --------------------------------------------------------------- PHASE 2 ----------------------------------------------------------------------
    print("phase 2: invert using attacker model")

    # pipe_provider_target.stash_pipe()
    res_2 = pipe_provider_target.invert_images(images=res_1["images_torch"],
    ...
    # log
    print(
        f"(Harmful image) detection_success: {detection_successful}, bit accuracy: {results['bit_accuracy']:.5f}, p_value: {results['p_value']}")
