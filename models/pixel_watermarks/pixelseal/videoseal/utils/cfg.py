...
def resolve_config_path(cfg_path):
    ...
    if not Path(cfg_path).is_file():
        # Use parents[1] to resolve relative to the pixelseal package root
...
def setup_model_from_checkpoint(ckpt_path: str) -> Videoseal:
    """
    # Example usage
    ckpt_path = '/path/to/pixelseal/checkpoint.pth'
    ...
    # Model cards are YAML files in pixelseal/cards/ directory
    elif not ckpt_path.endswith('.pth') and '/' not in ckpt_path:
        # This is likely a model card name (e.g., 'pixelseal', 'chunkyseal', 'pixelseal')
        return setup_model_from_model_card(ckpt_path)
    # load pixelseal ckpts
    else:
...
def maybe_download_checkpoint(url):
    try:
        ...
        # For URLs like .../pixelseal/chunkyseal/checkpoint.pth, use chunkyseal_checkpoint.pth
