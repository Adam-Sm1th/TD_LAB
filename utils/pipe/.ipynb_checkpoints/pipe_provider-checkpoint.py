...
class PipeProvider(ABC):
    ...
    def load_diffusers_pipe(self,
        ...
        if unet_id_or_checkpoint_dir is not None:
            kwargs_init["unet"] = self.get_unet(unet_id_or_checkpoint_dir)

        # plug vae
        if vae_id is not None:
            kwargs_init["vae"] = self.get_vae(vae_id)
        
        # load our custom diffusers pipe subclass
        pipe = self.call_from_pretrained(pretrained_model_name_or_path=pretrained_model_name_or_path, **kwargs_init)
        
        # --- VAE Memory Optimization ---
        if hasattr(pipe, "vae"):
            # VAE Slicing splits the batch into smaller steps
            pipe.vae.enable_slicing()
            # VAE Tiling splits the image into tiles (crucial for high-res)
    ...
    def vae_encode(self, images: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            images = 2. * images - 1.
            # Convert to target dtype to ensure consistency
            images = images.to(self.get_dtype())
    ...
    def stash_pipe(self):
        """Drop pipe to free up memory significantly"""
